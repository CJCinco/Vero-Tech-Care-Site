...
- Populate the setup dropdown only from a server-controlled catalog of workshops that already have a verified local `6 Sign Up Sheet/` destination and import allowlist entry. Restrict the unauthenticated catalog response to workshop facts already published on an approved public Vero Tech Care page; never include a private event, attendee fact, local path, credential, or internal event identifier. Require unique selection keys, labels, and derived event identifiers. The browser sends a non-secret selection key; the server resolves the canonical title and dated details before deriving the internal identifier. Do not provide a custom free-text fallback.
- Keep at most one workshop open at a time, enforced atomically by the activation write. Block opening a different workshop until the prior one is closed, require the setup screen to name the exact selected workshop before a close request is sent, and never remove an open workshop from the catalog until its close is confirmed.
